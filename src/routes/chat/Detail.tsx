        ...
        <div className="flex h-full transform-gpu flex-col items-center justify-center pb-16 transition-all duration-200 max-h-full sm:max-h-0 mb-0">
            <div className="flex flex-col items-center gap-0 p-2">
                <div className="flex flex-col md:flex-row items-center justify-center gap-2">
                    ...
                    <p className="py-1 split-parent inline overflow-hidden text-2xl sm:text-4xl font-medium text-text-primary"
                       aria-hidden="true"
                       style={{"textAlign": "center", "whiteSpace": "normal", "overflowWrap": "break-word"}}>Good
                        day, {session.firstName}</p>
                </div>
            </div>
        </div>
...
const Messages = memo((props: { data: Message[] }) => {
    return (
        <div className="relative flex-1 overflow-hidden overflow-y-auto">
            {props.data?.map((message) => <ChatMessage key={message.id}
                                                       data={message}/>)}
        </div>
    )
})
