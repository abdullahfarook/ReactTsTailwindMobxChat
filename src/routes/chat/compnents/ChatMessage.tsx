    ...
    return (
        <div className="text-token-text-primary w-full border-0 bg-transparent dark:border-0 dark:bg-transparent">
            <div className="m-auto justify-center p-4 py-2 md:gap-6">
                <div id={message.id} aria-label={`message-${message.id}`} className="group mx-auto flex flex-1 gap-3 transition-all duration-300 transform-gpu md:max-w-[47rem] xl:max-w-[55rem] focus:outline-none focus:ring-2 focus:ring-border-xheavy message-render">
                    ...
                    <div className={`relative flex w-11/12 flex-col ${isAgent ? 'agent-turn' : 'user-turn'}`}>
                            ...
                            <div className="flex max-w-full flex-grow flex-col gap-0">
                                <div className="text-message flex min-h-[20px] flex-col items-start gap-3 overflow-visible [.text-message+&amp;]:mt-5" dir="auto">
                                    <div className="markdown prose message-content dark:prose-invert light w-full break-words dark:text-gray-20">
                                        <Markdown isStreaming={!message.isComplete}>{message.content}</Markdown>
                                    </div>
                                </div>
                            </div>
                            <span></span>
