...
const ChatInput = () => {
    ...
    function scrollToBottomChat(className: string) {
        ...
        setTimeout(() => {
            const element = getLastElementByClass(className);
            if (element == null) {
                return;
            }
        ...
        <form
            ...
            <div className="relative flex h-full flex-1 items-stretch md:flex-col">
                <div className="flex w-full items-center">
                    <div
                        className="relative flex w-full flex-grow flex-col overflow-hidden rounded-t-3xl border pb-4 text-text-primary transition-all duration-200 sm:rounded-3xl sm:pb-0 shadow-md border-border-light bg-surface-chat">
                        <div className="flex flex-row">
                            <textarea onKeyDown={handleKeyDown}
                                      ...
                                      placeholder="Message Catalyst GPT"></textarea>
                        </div>
                        <div className="items-between flex gap-2 pb-2 flex-row">
                            <div className="mx-auto flex"></div>
                            <div className="mr-2">
                                <button onClick={submitMessage}
                                        disabled={chat.chatLoading || !value || value == ''}
                                        aria-label="Send message"
                                        id="send-button"
                                        className="cursor-pointer rounded-full bg-text-primary p-1.5 text-text-primary outline-offset-4 transition-all duration-200 disabled:cursor-not-allowed disabled:text-text-secondary disabled:opacity-10"
                                        data-testid="send-button"
                                        type="submit"
                                        aria-describedby=":ra:"><span
                                    className=""
                                    data-state="closed"><svg width="24"
                                                             height="24"
                                                             viewBox="0 0 24 24"
                                                             fill="none"
                                                             className="text-white dark:text-black"><path
