...
function SidePanel() {
    ...
    return <div data-testid="nav"
                className="nav active max-w-[320px] flex-shrink-0 transform overflow-x-hidden bg-surface-primary-alt transition-all duration-200 ease-in-out md:max-w-[260px]"
                style={navStyle}>
                        ...
                        <nav id="chat-history-nav"
                            ...
                            <div className="flex flex-1 flex-col">
                                ...
                                <div className="relative flex h-full flex-col pb-2 text-sm text-text-primary">
                                    <div className="flex-1 relative overflow-y-auto"
                                        ...
                                        {!chat.convsLoading && <Conversations data={chat.convsByDate}
                            ...
                            <button onClick={() => auth.logout()}
                                ...
                                <div className="-ml-0.9 -mt-0.8 h-8 w-8 flex-shrink-0">
                                    <div className="relative flex"><img className="rounded-full"
                                                                        ...
                                                                        alt={session.fullName}/></div>
                                </div>
                                <div className="mt-2 grow overflow-hidden text-ellipsis whitespace-nowrap text-left text-text-primary"
                                     style={{"marginTop": "0px", "marginLeft": "0px"}}>{session.fullName}</div>
                            </button>
                            <div style={{"position": "absolute", "top": "0px", "left": "0px", "width": "max-content"}}>
                                <div id=":rp:"
                                    ...
                                    <div className="text-token-text-secondary ml-3 mr-2 py-2 text-sm"
                                         role="note">sunny.rvd101@gmail.com
                                    </div>
                                    <div role="separator"
                                         aria-orientation="horizontal"
                                         className="-mx-1 my-1 h-px bg-border-medium"></div>
                                    <div id=":rs:"
                                         aria-selected="true"
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                             ...
                                             className="lucide lucide-file-text icon-md"
                                             aria-hidden="true">
                                    ...
                                    <div id=":r15:"
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                             ...
                                             strokeLinecap="round"
                                             strokeLinejoin="round"
                                             className="lucide lucide-log-out icon-md"
                                             aria-hidden="true">
...
const Conversations = (props: { data?: [string, TConversation[]][]; activeId?: string }) => {
    ...
    const navigator = useInstance(NavigationSrv);
    if (!data) return null;
    return (
        <>
            {data.map(([time, conversation]) =>
                <div key={time}
                    ...
                    <div className="mt-2 pl-2 pt-1 text-text-secondary"
                         style={{"fontSize": "0.7rem"}}>{time}</div>
                    {conversation.map((conv) =>
                        <NavItem key={conv.id}
                                 id={conv.id}
                                 title={conv.title}
                                 active={activeId == conv.id}
                                 onClick={() => navigator.navigate(`/chat/${conv.id}`)}/>)}
                </div>
            )}
        </>
    );
