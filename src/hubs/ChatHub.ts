...
export class ChatHub {
  ...
  public async sendInferenceRequestAsync(): Promise<void> {
    if (!this.hubConnection || !this.isConnected) {
      throw new Error('Hub connection is not established');
    }

    const inferenceRequest = await this.getInferenceRequestFromWebsocketMessages();
  ...
  private async getInferenceRequestFromWebsocketMessages(): Promise<InferenceRequest> {
